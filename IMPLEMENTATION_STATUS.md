...
> **Status update (2026-06-15 / v1.1.1):** the **decoder** is complete for the
...
> (`TestCGORef`, `-tags opusref`). The **encoder** is a CELT-only quality
> pipeline that emits standard Opus packets libopus can decode, but it is not
> bit-exact with libopus and does not yet implement SILK-only or hybrid encode.
...
- ✅ **Encoder**: CELT-only quality pipeline with transient handling, TF analysis, allocation shaping, stereo/intensity decisions, bandwidth detection, VBR/CVBR, DTX, and multi-frame packetization. Output is standard Opus but not bit-exact with libopus.
- ⚠️ SILK/hybrid encode, true packet FEC extraction, public PLC, multistream, surround, and Ogg Opus container APIs remain incomplete.

## Spec Gaps (prioritized)
1. **SILK/hybrid encoder modes**: top-level mode selection and packet generation beyond CELT-only.
