...
マルチストリーム packet は RFC 6716 の self-delimited framing を使用し、PLC・
in-band FEC decode を含む libopus 1.6.1 相互運用テストを通過しています。
サラウンドも同じ decoder method を公開し、mapping family 0、1（Vorbis順、
最大7.1）、255に対応します。
...
packet timing trim、chained logical stream の自動読取り、seekable input の
`(*Reader).SeekPCM` を提供します。`Writer` は1つの logical stream を書きます。
...
- Ogg Opus reader は chained logical stream と link 単位の sample-accurate seek
  に対応しますが、multiplexed stream demux は提供しません。
