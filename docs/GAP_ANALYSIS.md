...
- **2026-09-25 完了（restricted application・bitrate 下限・文書整理）：** libopus 1.6 の `ApplicationRestrictedSILK` / `ApplicationRestrictedCELT` を追加（生成時に固定、常に libopus policy）。sweep の restricted 群 144 構成と multistream 18 構成が byte 一致。`SetBitrate` は libopus と同じく正の値を [500, 750000×ch] に clamp（6000 未満の拒否を廃止）。CTL_PARITY・MODE_RATE_POLICY_DIFF・CURRENT_IMPLEMENTATION の Legacy 前提の古い記述を整理
- **2026-09-24 完了（multistream・surround・projection）：** `SetModePolicy(ModePolicyLibopus)` を multistream / surround / projection encoder に追加し `opus_multistream_encode_native` を移植（rate allocation、stream ごとの予算、repacketizer framing）。family 1 の surround 解析と energy mask（CELT の VBR・dynalloc・trim、SILK rate）、LFE stream、family 2/3 の CELT 強制と float32 mixing を移植。途中で見つかった CELT の不一致（reset で mask を消す、無音フレームの transient_got_disabled、bitrate MAX の VBR、hybrid 後の HB_gain fade、500 b/s 以下の CELT 配分、SILK 5000 b/s 未満）も修正。`TestMultistreamEncoderOracle` 171 構成すべて byte 一致
- **2026-09-24 完了（20 ms 未満のパケット）：** `ModePolicyLibopus` で全パケット長を libopus の判定に通すよう変更。2.5/5 ms（常に CELT-only）と 10 ms の CELT-only に加え、10 ms の SILK/hybrid に対応（`silk.Encoder.SetFrameMs` で `silk_setup_fs` と同じく状態を保ったまま 10/20 ms を切替、SILK prefill は常に 10 ms）。CELT の stereo 幅をパケットの `equiv_rate` で計算、空予算でも bit 割り当てを最後まで実行、libopus の「PLC フレーム」（予算不足時の TOC のみパケット）を移植、フレーム長切替で CELT の損失率が失われる不具合を修正。sweep に短いフレームの遷移・DTX・FEC を追加し 2796 構成すべて byte 一致
- **2026-09-23 完了（CI gate・sweep・arm64 決定性）：** opusref CI が計装 oracle を Linux でビルドし、CELT/hybrid/SILK/自動モード/sweep/遷移/FEC/DTX の oracle テストを実行（oracle ビルドスクリプトをクロスプラットフォーム化、glibc の `-nan` 表記にパーサを対応）。`TestAutoModeOracleSweep` を常時 gate 化（2340 構成：全ビットレート段階 × CBR/CVBR/UVBR × complexity 0/3/7/10 × voice/music/auto、8〜24 kHz 入力、40〜120 ms、帯域強制・上限、mono 強制、int16 入力、LSB 深度、8/12/24 kHz の FEC・DTX）で全 byte 一致。sweep で `OPUS_SET_MAX_BANDWIDTH(MEDIUMBAND)` の上限が CELT 用マッピング（WB）になっていた不具合を発見・修正。arm64 等で gc が積和を FMA に融合し float32 移植の出力がアーキテクチャ依存になっていたため、加減算に入る積をすべて明示変換で丸め（amd64 では無変化）、arm64 ビルドに FMA があれば CI が失敗するようにした。並列化で見つかったパッケージ変数のデータ競合も修正
