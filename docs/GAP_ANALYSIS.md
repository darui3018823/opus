...
### Q1 残：LPC/NLSF 解析の完全ポート【完了】

- **状態：** 解決済み（libopus 1.5.2 / 1.6 完全互換・oracle ビット完全一致実証済み）
- **実装済み：**
  - `silk_find_LPC_FLP` の完全ポート（全フレーム Burg AR + 後半 10ms Burg AR + 第1ハーフ残差基準 + `k=3..0` 補間探索 + `silkLPCAnalysisFilterFLP` 残差判定）
  ...
  - 恣意的なガード条件・legacy 全探索（`guardedFaithfulBurgNLSFAnalysis`, `bestNLSFAnalysis` 等）の完全撤廃
