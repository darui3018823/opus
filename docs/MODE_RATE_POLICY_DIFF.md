...
| Hybrid split | libopus subtracts SILK rate from total rate, lets CELT choose the hybrid VBR size after its header analysis, calls `ec_enc_shrink`, then performs allocation. | Hybrid writes SILK first into one shared range stream. For non-redundant VBR/CVBR, CELT derives its share from the actual SILK prefix rate, applies the existing high-band activity calibration, clamps against the post-header `min_allowed`, shrinks, and only then allocates pulses. Final range and libopus cross-decode regressions cover hard-onset nominal overshoot. CBR and redundancy remain fixed-size. The broader SILK target-rate policy is still simplified. | Partial |
