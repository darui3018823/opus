...
released baseline is `v1.4.1`; do not infer the next version from that fact.
...
- Inventory everything since the baseline, including exported API additions,
  ...
  ```powershell
  git log --first-parent --oneline v1.4.1..HEAD
...
The `v1.4.1` release included backward-compatible public additions such as
