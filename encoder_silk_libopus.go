...
func (e *Encoder) silkInternalRateControl(frameSize, desiredRate int, silkPrefill []float64, prefill2 bool,
	...
	if silkPrefill != nil {
		if prefill2 {
			// The switch itself: the control pass with opusCanSwitch runs on
			// the encoder that ran the transition (libopus: on the re-initialised
			// state with saved_fs_kHz as the previous rate), then the encoder is
			// re-initialised at the new rate with the transition filter carried
			// over.
			fs, _ := e.silkEncoder.ControlAudioBandwidth(desiredRate, true)
			if fs*1000 != e.silkSampleRate {
				old := e.silkEncoder
				oldPrevChannels := e.silkPrevChannels
				if err := e.rebuildSILKEncoder(fs * 1000); err != nil {
					return false, err
				}
				if err := reapply(); err != nil {
					return false, err
				}
				// silk_init_encoder resets the channel states only.
	...
	fs, ready := e.silkEncoder.ControlAudioBandwidth(desiredRate, false)
	if fs*1000 != e.silkSampleRate {
...
// before a SILK bandwidth switch). CBR packets are padded to cbr_bytes.
func (e *Encoder) encodeSILKOnlyPacketLibopus(pcm, celtPCM []float64, d modeDecision, maxDataBytes int,
	...
	if silkPrefill != nil && !prefill2 {
		// CELT-only -> SILK: silk_InitEncoder.
		e.silkFramesCoded = false
	}
	if !e.silkFramesCoded && (silkPrefill != nil || e.silkSampleRate != desiredRate) {
	...
	bits := bitrateToBits(e.bitrate, e.sampleRate, frameSize)
	...
	toc, err := framing.GenerateTOCExt(framing.ModeSILKOnly, bw, streamChannels, framing.FrameSize20ms)
	if err != nil {
		return nil, fmt.Errorf("failed to generate SILK TOC: %w", err)
	}

	silkFrameSize := e.silkSampleRate * 20 / 1000
	silkPCM := e.silkInput(pcm, silkFrameSize*streamChannels)
	e.inDTX = false
	...
	if switchReady {
		...
		redBytes = computeRedundancyBytes(maxDataBytes, e.bitrate, frameRate, streamChannels)
...
func (e *Encoder) silkOnlyLibopusPath(nFrames int) bool {
	return e.libopusModePolicy && nFrames == 1 && e.silkEncoder != nil
}
