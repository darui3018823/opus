...
// nlsf_cb.go — NLSF codebook data ported verbatim from libopus
// silk/tables_NLSF_CB_NB_MB.c and silk/tables_NLSF_CB_WB.c

// ── NB/MB codebook (order=10) ────────────────────────────────────────────────

// silkNLSFCB1NBMBQ8 — stage-1 codebook, 32 entries × 10 coefficients, Q8.
// From silk_NLSF_CB1_NB_MB_Q8[320].
var silkNLSFCB1NBMBQ8 = [32][10]uint8{
...
// silkNLSFCB1ICDFNBMB — stage-1 iCDF for NB/MB (64 values = 2×32 split into two halves).
// From silk_NLSF_CB1_iCDF_NB_MB[64].
var silkNLSFCB1ICDFNBMB = [64]uint8{
...
// silkNLSFCB2ICDFNBMB — stage-2 iCDF for NB/MB (72 values = 8 rows × 9 values).
// From silk_NLSF_CB2_iCDF_NB_MB[72].
var silkNLSFCB2ICDFNBMB = [72]uint8{
...
// silkNLSFPredNBMBQ8 — NLSF prediction coefficients for NB/MB (18 values, Q8).
var silkNLSFPredNBMBQ8 = [18]uint8{
	179, 138, 140, 148, 151, 149, 153, 151,
	163, 116, 67, 82, 59, 92, 72, 100,
	89, 92,
}

// silkNLSFDeltaMinNBMBQ15 — minimum NLSF delta for NB/MB (11 values, Q15).
var silkNLSFDeltaMinNBMBQ15 = [11]int16{
	250, 3, 6, 3, 3, 3, 4, 3, 3, 3, 461,
}

// ── WB codebook (order=16) ───────────────────────────────────────────────────

// silkNLSFCB1WBQ8 — stage-1 codebook, 32 entries × 16 coefficients, Q8.
// From silk_NLSF_CB1_WB_Q8[512].
var silkNLSFCB1WBQ8 = [32][16]uint8{
...
// silkNLSFCB1ICDFWB — stage-1 iCDF for WB (64 values = 2×32 halves).
// From silk_NLSF_CB1_iCDF_WB[64].
var silkNLSFCB1ICDFWB = [64]uint8{
...
// silkNLSFCB2ICDFWB — stage-2 iCDF for WB (72 values = 8 rows × 9 values).
// From silk_NLSF_CB2_iCDF_WB[72].
var silkNLSFCB2ICDFWB = [72]uint8{
...
// From silk_NLSF_CB2_SELECT_WB[256].
var silkNLSFCB2SelectWB = [256]uint8{
	0, 0, 0, 0, 0, 0, 0, 1,
...
// silkNLSFPredWBQ8 — NLSF prediction coefficients for WB (30 values, Q8).
var silkNLSFPredWBQ8 = [30]uint8{
...
// silkNLSFDeltaMinWBQ15 — minimum NLSF delta for WB (17 values, Q15).
var silkNLSFDeltaMinWBQ15 = [17]int16{
	100, 3, 40, 3, 3, 3, 5, 14,
	14, 10, 11, 3, 8, 9, 7, 3,
	347,
}

// ── Weight tables from libopus (silk_NLSF_CB1_Wght_Q9) ──────────────────────

// silkNLSFCB1WghtNBMBQ9 — NB/MB weight table, 32 entries × 10 coefficients, Q9.
// From silk_NLSF_CB1_Wght_Q9[320] in tables_NLSF_CB_NB_MB.c
var silkNLSFCB1WghtNBMBQ9 = [32][10]int16{
...
// silkNLSFCB1WghtWBQ9 — WB weight table, 32 entries × 16 coefficients, Q9.
// From silk_NLSF_CB1_WB_Wght_Q9[512] in tables_NLSF_CB_WB.c
var silkNLSFCB1WghtWBQ9 = [32][16]int16{
...
var silkNLSFCB1NBMBQ8Flat [32 * 10]uint8
var silkNLSFCB1WBQ8Flat [32 * 16]uint8
var silkNLSFCB1WghtNBMBQ9Flat [32 * 10]int16
var silkNLSFCB1WghtWBQ9Flat [32 * 16]int16

func init() {
	// Flatten NB/MB CB1
	for i := 0; i < 32; i++ {
		for j := 0; j < 10; j++ {
			silkNLSFCB1NBMBQ8Flat[i*10+j] = silkNLSFCB1NBMBQ8[i][j]
			silkNLSFCB1WghtNBMBQ9Flat[i*10+j] = silkNLSFCB1WghtNBMBQ9[i][j]
		}
	}
	// Flatten WB CB1
	for i := 0; i < 32; i++ {
		for j := 0; j < 16; j++ {
			silkNLSFCB1WBQ8Flat[i*16+j] = silkNLSFCB1WBQ8[i][j]
			silkNLSFCB1WghtWBQ9Flat[i*16+j] = silkNLSFCB1WghtWBQ9[i][j]
		}
	}
}

// getNLSFCB returns the codebook parameters for a given LPC order.
func getNLSFCB(order int) *nlsfCBParams {
	if order == 16 {
		return nlsfCBWB
	}
	return nlsfCBNBMB // order 10 (NB) or 12 (MB, same codebook)
}
