...
const silkQualityFrames = 12

type silkQualitySignal struct {
	name          string
...
type silkQualityMetrics struct {
...
func TestSILKInternalQualityBaseline(t *testing.T) {
	const rate = 16000
	const frameSize = rate * 20 / 1000

	for _, sig := range silkQualitySignals() {
		t.Run(sig.name, func(t *testing.T) {
			...
			for frame := 0; frame < silkQualityFrames; frame++ {
				...
				if len(pkt) == 0 {
					t.Fatalf("frame %d: empty packet", frame)
				}
			...
			m := measureSILKQuality(in, out, frameSize, totalPacketBytes, sig.pitchTracked)
			t.Logf("%s: packet=%.1fB outRMS=%.5f peak=%.4f clips=%d alignedSNR=%.2fdB rmse=%.5f delay=%d scale=%.4f pitchMean=%.1f pitchMaxDelta=%.1f",
				sig.name, m.avgPacketBytes, m.outRMS, m.outPeak, m.clipCount, m.alignedSNR, m.alignedRMSE, m.delay, m.scale, m.pitchMean, m.pitchMaxDelta)
			assertSILKQuality(t, sig, m)
		})
	}
}

func silkQualitySignals() []silkQualitySignal {
	return []silkQualitySignal{
		{
			name:          "silence",
			silent:        true,
			maxSilenceRMS: 0.015,
		...
		},
		{
			...
			gen: func(rate, start, n int) []float64 {
				out := make([]float64, n)
				rng := rand.New(rand.NewSource(0x51514 + int64(start/rate)))
				prev := 0.0
				for i := range out {
		...
		},
		{
			...
			minSNR:       -20,
			gen: func(rate, start, n int) []float64 {
				return silkHarmonicFrame(rate, start, n, 180, 0.20)
			},
		},
		{
			...
			gen: func(rate, start, n int) []float64 {
				...
				for i := range out {
					...
					f0 := 145 + 24*math.Sin(2*math.Pi*1.7*t)
					env := 0.18 + 0.10*math.Sin(2*math.Pi*3.1*t+0.2)
					out[i] = env * (0.58*math.Sin(2*math.Pi*f0*t) +
						0.24*math.Sin(2*math.Pi*2*f0*t+0.35) +
						0.11*math.Sin(2*math.Pi*3*f0*t+0.85))
				}
				return out
			},
		},
		{
			...
			minSNR: -24,
			gen: func(rate, start, n int) []float64 {
				out := silkHarmonicFrame(rate, start, n, 220, 0.20)
				for i := range out {
					global := start + i
					switch {
					case global < 2*n:
...
func silkHarmonicFrame(rate, start, n int, f0, amp float64) []float64 {
	out := make([]float64, n)
	for i := range out {
		t := float64(start+i) / float64(rate)
		out[i] = amp * (0.72*math.Sin(2*math.Pi*f0*t) +
			0.22*math.Sin(2*math.Pi*2*f0*t+0.3) +
			0.09*math.Sin(2*math.Pi*3*f0*t+0.7))
	}
	return out
}

func measureSILKQuality(in, out []float64, frameSize, totalPacketBytes int, pitchTracked bool) silkQualityMetrics {
	m := silkQualityMetrics{
		avgPacketBytes: float64(totalPacketBytes) / silkQualityFrames,
	}
	m.outRMS, m.outPeak, m.clipCount = silkQualityStats(out)
	m.alignedSNR, m.alignedRMSE, m.delay, m.scale = silkAlignedSNR(in, out, frameSize)
	if pitchTracked {
		m.pitchMean, m.pitchMaxDelta = silkPitchContinuity(out, frameSize, 40, 180)
	}
	return m
}

func assertSILKQuality(t *testing.T, sig silkQualitySignal, m silkQualityMetrics) {
	t.Helper()
...
func silkQualityStats(x []float64) (rms, peak float64, clipCount int) {
	for _, v := range x {
		rms += v * v
		a := math.Abs(v)
		if a > peak {
...
func silkAlignedSNR(ref, out []float64, maxDelay int) (snr, rmse float64, delay int, scale float64) {
	bestRMSE := math.Inf(1)
	bestRefRMS := 0.0
	bestDelay := 0
	bestScale := 1.0
...
func silkPitchContinuity(x []float64, frameSize, minLag, maxLag int) (mean, maxDelta float64) {
	var lags []float64
	for start := 0; start+frameSize <= len(x); start += frameSize {
		lag, corr := silkBestPitchLag(x[start:start+frameSize], minLag, maxLag)
		if corr >= 0.25 {
