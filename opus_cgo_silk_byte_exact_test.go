...
// identical settings — VOIP, constrained VBR or CBR, complexity 5, voice
// signal, with and without in-band FEC at 20 % loss — and requires byte-identical packets
...
func TestCGOEncodeRefSILKByteExact(t *testing.T) {
	...
	for _, mode := range []string{"cvbr", "cbr"} {
		for _, fec := range []bool{false, true} {
			for _, c := range cells {
				channels, rate, bitrate := c.channels, c.rate, c.bitrate
				if mode == "cbr" && fec {
					continue // FEC is exercised in CVBR; CBR cells cover the quantiser loop
				}
				name := mode + "/nofec"
				if fec {
					name = mode + "/fec20"
				...
				t.Run(fmt.Sprintf("%s/%dch/%dk/%dkbps", name, channels, rate/1000, bitrate/1000), func(t *testing.T) {
					...
					if err := ref.SetVBR(mode != "cbr"); err != nil {
						t.Fatal(err)
					}
